...
/*
  target
  이벤트 핸들러 함수의 매개변수 e 는 이벤트 객체를 나타냅니다.
  이 객체에는 이벤트와 관련된 다양한 정보가 포함되어 있는데
  그중 target의 속성은 매우 중요합니다.

  e.target은 이벤트가 발생한 HTML요소를 나타냅니다.
  이 코드에서는 클릭된 Add contact 버튼 자체를 가리킵니다.

  따라서 e.target.textContent를 사용하면 클릭된 버튼의 텍스트 내용을
  변경할 수 있습니다. 이를 통해 버튼의 상태(Added/Add contact)를
  토글할 수 있습니다.

  요약하면, 이 코드는 Add contact 버튼을 클릭할 때마다 
  버튼의 텍스트와 CSS클래스를 변경하여 버튼의 상태를 
  토글하는 기능을 구현한 것입니다.
...
/*
e.target을 사용하지 않으면 이베트 핸들러 함수에서 이벤트가 발생한 HTML요소를 
참조할 수 없습니다.

e.target은 이벤트가 발생한 실제 HTML요소를 가리키는  반면, e.currentTarget은 이벤트 리스너가 등록된 요소를 가리킵니다. 

따라서 e.target을 사용하지 않으면 클릭된 버튼의 텍스트 내용을 변경할 수 없습니다. 대신 e.currentTarget을 사용하면 이벤트 리스너가 등록된 요소의 텍스트 내용을 변경할 수 있지만, 이는 의도한 동작과 다를 수 있습니다. 

예를 들어, 이벤트 리스너가 상위 요소에 등록되어 있다면 e.currentTarget을 사용하면 상위 요소의 텍스트가 변경될 것입니다. 이는 사용자 경험 측면에서 바람직하지 않을 수 있습니다. 

따라서 이벤트 핸들러 함수에서 이벤트가 발생한 실제 HTML 요소를 참조하려면 e.target을 사용해야 합니다. 이를 통해 사용자의 의도에 맞는 동작을 구현할 수 있습니다
...
/*
  JS 에서 이벤트 핸들러는 웹페이지나 앱에서 발생하는 이벤트(예: 클릭, 마우스 이동, 키 누름 등)을 처리한 함수를 가리킵니다.
  이벤트 핸들러는 특정 이벤트가 발생했을때 실행되도록 설정되어 있으며, 
  해당 이베트에 대한 반응을 정의하는 역할을 합니다.
...
target은 HTML 요소의 속성 중 하나로, 링크된 문서가 열리는 위치를 지정하는 역할을 합니다.

구체적으로 target 속성이 가질 수 있는 값은 다음과 같습니다:
...
이를 통해 웹 페이지에서 링크된 문서가 어떤 방식으로 열릴지 제어할 수 있습니다. 예를 들어 `target="_blank"`를 사용하면 링크가 새 탭이나 창에서 열리게 됩니다. 

따라서 target 속성은 HTML 문서 간 이동 방식을 지정하는 데 사용되는 중요한 속성이라고 할 수 있습니다.
